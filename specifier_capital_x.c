...
int specifier_capital_x(va_list ap, char *save_f, int *w_n_p, char *save_l)
{
    ...
    if (count4(num, w_n_p) >= w_n_p[0] && count4(num, w_n_p) >= w_n_p[1])
        ...
        i = count4(num, w_n_p) + capital_x_w_is_biggest_p_is_second_with_m(save_f, w_n_p, num);
    else if (w_n_p[0] > count4(num, w_n_p) && w_n_p[0] > w_n_p[1] && w_n_p[1] >= count4(num, w_n_p))
        i = count4(num, w_n_p) + capital_x_w_is_biggest_p_is_second(save_f, w_n_p, num);
    else if (w_n_p[0] > count4(num, w_n_p) && w_n_p[0] > w_n_p[1] && w_n_p[1] <= count4(num, w_n_p) && flag_check(save_f, '-'))
        i = count4(num, w_n_p) + capital_x_w_is_biggest_c_is_second_with_m(save_f, w_n_p, num);
    else
        i = count4(num, w_n_p) + capital_x_w_is_biggest_c_is_second(save_f, w_n_p, num);
    return (i);
}